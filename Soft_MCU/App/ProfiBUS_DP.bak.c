...
#define PBDP_EVENT_RECV     (0x1u << 3)             /* PBDP Event Recv Char             */
#define PBDP_EVENT_SEND     (0x1u << 4)             /* PBDP Event Send Char             */
#define PBDP_EVENT_SDCP     (0x1u << 5)             /* PBDP Event Send complete         */
...
typedef struct {    /*------------- PBDP Information (Run-Time) ----------------------------*/
    uint32_t                                idl_cnt;    /* Number of sequential idle char   */
    ...
    uint32_t                                err_ovr;    /* Number of Queue Overrun error    */
    uint32_t                                err_chk;    /* Number of transmit               */
    QUEUE_TYPE(uint8_t, 1024)               dbg_evt;    /* Event debug Queue                */
} PBDP_INFO;
...
int PBDP_Recv(uint8_t buff[260])
{
    ...
    if( osOK != osMutexWait(PBDP_Info.rx_mut, osWaitForever) )  return( 0 );    /* osMutexWait Error        */
    if( osSemaphoreWait(PBDP_Info.rx_sem, osWaitForever) <= 0)  return( 0 );    /* Waiting for Received ED  */
...
#       define  GOTO_DEL(ret)   { result = ret;  goto RECV_DEL; }
        ...
        if(  (rx_len = QUEUE_SIZE(PBDP_Info.rx_que)) <= 0 ) {    GOTO_RET(0); } /* Receiver Queue is empty  */

        switch( QUEUE_GET(PBDP_Info.rx_que, 0) ) {
        case PBDP_FRAME_SD1:    /*--------------------- DLPDUs of fixed length with no data field ----------*/
            if( rx_len <= PBDP_FRAME_SD1L ) { /* Return FALSE */ GOTO_RET(0); } /* Less the Frame data Len  */
            if( PBDP_FRAME_SD1_FORMAT_CHECK(PBDP_FRAME_SD1L) ) { GOTO_DEL(0); } /* Frame format invalid     */
        ...
        case PBDP_FRAME_SD2:    /*--------------------- DLPDUs with variable data field length -------------*/
            ...
            if( rx_len <= result ) { /*--------- Return FALSE */ GOTO_RET(0); } /* Less the Frame data Len  */
            if( PBDP_FRAME_SD2_FORMAT_CHECK(result) ) {          GOTO_DEL(0); } /* Frame format invalid     */
        ...
        case PBDP_FRAME_SD3:    /*--------------------- DLPDUs of fixed length with data field -------------*/
            if( rx_len <= PBDP_FRAME_SD3L ) { /* Return FALSE */ GOTO_RET(0); } /* Less the Frame data Len  */
            if( PBDP_FRAME_SD3_FORMAT_CHECK(PBDP_FRAME_SD3L) ) { GOTO_DEL(0); } /* Frame format invalid     */
        ...
        case PBDP_FRAME_SD4:    /*--------------------- token DLPDU ----------------------------------------*/
            if( rx_len <= PBDP_FRAME_SD4L ) { /* Return FALSE */ GOTO_RET(0); } /* Less the Frame data Len  */
            if( PBDP_FRAME_SD4_FORMAT_CHECK(PBDP_FRAME_SD4L) ) { GOTO_DEL(0); } /* Frame format invalid     */
        ...
        case PBDP_FRAME_SC:     /*--------------------- Short Acknowledgement ------------------------------*/
            if( rx_len <= PBDP_FRAME_SCL ) { /* Return FALSE  */ GOTO_RET(0); } /* Less the Frame data Len  */
            if( PBDP_FRAME_SC_FORMAT_CHECK(PBDP_FRAME_SCL) ) {   GOTO_DEL(0); } /* Frame format invalid     */
        ...
        default:                /*--------------------- End_Delimiter and Other Character ------------------*/
          RECV_DEL:  QUEUE_DEL(PBDP_Info.rx_que, 1);                   /* Queue Delete             */
    ...
    if( osOK != osMutexRelease(PBDP_Info.rx_mut) )  return( 0 );                /* osMutexRelease Error     */
    return( result );
}
...
int PBDP_Send(const uint8_t *buff, int len)
{
    ...
    if( osOK != osMutexWait(PBDP_Info.tx_mut, osWaitForever ) {     /* osMutexWait Error            */
        len = 0;    goto SEND_RET;
    }
    ...
    if( (osEventSignal != evt.status) || (PBDP_EVENT_TRCP != evt.value.signals) ) {                            /* PBDP Event error             */
        len = 0;    goto SEND_RET;                                          /* */
    }

    /*------------------------------------------------------------------------------------------*/
    evt = osSignalWait(0, osWaitForever);                                   /* Waiting for UART Event       */
    if( (osEventSignal != evt.status) || (PBDP_EVENT_SDCP != evt.value.signals) ) {
        len = 0;  //goto SEND_RET;
    }
    SEND_RET:
    PBDP_Info.tx_chk = 0;                                                   /* Number of data check         */
    PBDP_Info.tx_cnt = 0;                                                   /* Number of data transmitted   */
    PBDP_Info.tx_num = 0;                                                   /* Total  of transmit buffer    */
    PBDP_Info.tx_buf = NULL;                                                /* PBDP is Recv status          */

    PBDP_Info.tx_sig = NULL;
    if( osOK != osMutexRelease(PBDP_Info.tx_mut) ) {
        len = 0;
    }
    return( len );
    ...
    if( osOK != osMutexRelease(PBDP_Info.tx_mut) )  goto SEND_ERR;          /* osMutexRelease Error         */

    if( osEventSignal != evt.status ) /*--------*/  goto SEND_ERR;          /* osSignalWait Error           */
    if( PBDP_EVENT_TRCP != evt.value.signals )      goto SEND_ERR;          /* PBDP Event error             */
    return( len );

    SEND_ERR:  // printf("[ProfiBUS DP] Failed to Send!\r\n");
    return( 0 );
}
...
{
    PBDP_Info.idl_cnt = 0;                                      /* Clear Counter of Received IDLE       */

    if( QUEUE_FULL(PBDP_Info.dbg_evt) ) {
        QUEUE_DEL(PBDP_Info.dbg_evt, 1);
    }
    QUEUE_PUSH(PBDP_Info.dbg_evt) = PBDP_EVENT_RECV;
    ...
    if( PBDP_Info.tx_buf == NULL ) {    /*-------------- PBDP is Recv status ---------------------------*/
        ...
        if( PBDP_Info.tx_chk == PBDP_Info.tx_num ) {
            if( PBDP_Info.tx_sig ) {
                osSignalSet(PBDP_Info.tx_sig, PBDP_EVENT_SDCP); /* Send complete                        */
            }
            if( QUEUE_FULL(PBDP_Info.dbg_evt) ) {
                QUEUE_DEL(PBDP_Info.dbg_evt, 1);
            }
            QUEUE_PUSH(PBDP_Info.dbg_evt) = PBDP_EVENT_SDCP;
        }
    }
}
...
int PBDP_UART_SendCB(void)
{
    ...
    QUEUE_PUSH(PBDP_Info.dbg_evt) = PBDP_EVENT_SEND;
...
void PBDP_UART_EventCB(int event)
{
    if( event & PBDP_EVENT_ERR ) {  /*-------------- PBDP Event error ------------------------------*/
        ...
        QUEUE_PUSH(PBDP_Info.dbg_evt) = PBDP_EVENT_ERR;
    }
    ...
    if( event & PBDP_EVENT_IDLE ) { /*-------------- PBDP Event Recv Idle char ---------------------*/
        ...
        QUEUE_PUSH(PBDP_Info.dbg_evt) = PBDP_EVENT_IDLE;
    }

    if( event & PBDP_EVENT_TRCP ) { /*-------------- PBDP Event Transmission complete --------------*/
        if( PBDP_Info.tx_sig ) {
            osSignalSet(PBDP_Info.tx_sig, PBDP_EVENT_TRCP);     /* Set the signal flags             */
        }
      //PBDP_Info.idl_cnt = 0;                                  /* Clear Counter of Received IDLE   */
        if( QUEUE_FULL(PBDP_Info.dbg_evt) ) {
            QUEUE_DEL(PBDP_Info.dbg_evt, 1);
        }
        QUEUE_PUSH(PBDP_Info.dbg_evt) = PBDP_EVENT_TRCP;
    }
}
...
static uint32_t     s_ErrPE = 0, s_ErrFE = 0, s_ErrNE = 0, s_ErrORE = 0;

/**
  * @brief      UART Initialize
  * @param[in]  BaudRate    UART BaudRate
  * @param[in]  pSignal     pointer of signal flags
  */
void PBDP_UART_Init(uint32_t BaudRate)
{
    MODIFY_REG(RCC->AHB1ENR,   RCC_AHB1ENR_GPIOCEN              /*------ Init TxD(PC6) and RxD(PC7) --------*/
    ...
    MODIFY_REG(RCC->AHB1ENR,   RCC_AHB1ENR_GPIODEN              /*------ Init TxEN(PD7) --------------------*/
    ...
    MODIFY_REG(RCC->APB2ENR,   RCC_APB2ENR_USART6EN             /*------ Init Serial Interface -------------*/
                           ,   RCC_APB2ENR_USART6EN);           /* Enable USART6 clock                      */
    ...
    _hUART->Instance =  USART6;                                 /* Init UART registers base address         */
    NVIC_SetPriority(USART6_IRQn, 2);                           /* Set the USART6 priority                  */
    NVIC_EnableIRQ(USART6_IRQn);                                /* Enable the USART6 global Interrupt       */

    MODIFY_REG(RCC->APB1ENR,   RCC_APB1ENR_TIM3EN               /*------ Init Timer 3 ----------------------*/
                           ,   RCC_APB1ENR_TIM3EN);             /* Enable Timer 3 clock                     */
...
void USART6_IRQHandler(void)
{
    ...
    if( (tmp1 != RESET) && (tmp2 != RESET) ) {          /* USART Transmission complete                      */
      ...
      //TIM3->CR1 &= ~TIM_CR1_CEN;                      /* ---Timer Counter disable                         */
      //NVIC_ClearPendingIRQ(TIM3_IRQn);                /* ---Timer Pending Interrupt clear                 */
        PBDP_UART_EventCB(PBDP_EVENT_TRCP);
    }
    ...
    if( (tmp1 != RESET) && (tmp2 != RESET) ) {          /* USART Transmit data register empty               */
        if( (tmp3 = PBDP_UART_SendCB()) >= 0 )  { _hUART->Instance->DR = tmp3; }
        TIM3->SR   = ~TIM_SR_UIF;                       /* ---Timer Update interrupt flag clear             */
        TIM3->CR1 &= ~TIM_CR1_CEN;                      /* ---Timer Counter disable                         */
        NVIC_ClearPendingIRQ(TIM3_IRQn);                /* ---Timer Pending Interrupt clear                 */
    }
