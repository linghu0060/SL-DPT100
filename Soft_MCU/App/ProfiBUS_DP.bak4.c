...
#define PBDP_RX_QUE_PUSH(val)   {   if(!QUEUE_FULL(PBDP_Info.rx_que) ) {                    \
...
int PBDP_Send(const uint8_t *buff, int len)
{
    ...
    switch( buff[0] ) {
    case PBDP_FRAME_SD1:  cnt = (buff[3] & 0x40) ? (3) : (2);   break;
    case PBDP_FRAME_SD2:  cnt = (buff[6] & 0x40) ? (3) : (2);   break;
    case PBDP_FRAME_SD3:  cnt = (buff[3] & 0x40) ? (3) : (2);   break;
    case PBDP_FRAME_SD4:  cnt = 3; /************************/   break;
    case PBDP_FRAME_SC:   cnt = 2; /************************/   break;
    default: /**********************************************/   return( -1 );
    }
    cnt += 0;//g_count;// + (osKernelSysTick() & 0x01);
...
static uint16_t                             TIME_ARR[2];                /* Timer Auto-reload value      */
...
#define PBDP_UART_IDEL_CHK_EN()    // (TIM3->CR1 |=  TIM_CR1_CEN, TIM3->SR = ~TIM_SR_UIF, TIM3->EGR |= TIM_EGR_UG)
#define PBDP_UART_IDEL_CHK_DS()    // (TIM3->CR1 &= ~TIM_CR1_CEN, TIM3->SR = ~TIM_SR_UIF)
#define PBDP_UART_IDEL_LED_TURN()   (  (GPIOD->ODR   & (0x1u << 1*13))/* Turn of UART Idel Status LED       */ \
                                     ? (GPIOD->BSRRH = (0x1u << 1*13))                                         \
                                     : (GPIOD->BSRRL = (0x1u << 1*13))                                         \
                                    )
#define PBDP_UART_IDLE_CHK_RST()    (TIM3->SR = ~TIM_SR_UIF, TIM3->EGR |= TIM_EGR_UG)
...
void PBDP_UART_Init(uint32_t BaudRate)
{
    ...
    MODIFY_REG(RCC->APB2ENR,   0                                /* Enable USART6 clock                      */
    ...
    MODIFY_REG(RCC->AHB1ENR,   0                                /* Enable GPIOD clock                       */
                           ,   RCC_AHB1ENR_GPIODEN);
    MODIFY_REG(GPIOD->MODER,   (0x3u << 2*13)                   /* PD13 Output mode                         */
                           ,   (0x1u << 2*13));
    WRITE_REG( GPIOD->BSRRL,   (0x1u << 1*13));                 /* PD13 Output High                         */
    MODIFY_REG(GPIOD->OTYPER,  (0x1u << 1*13)                   /* PD13 Output push-pull                    */
                            ,  (0x0u << 1*13));
    MODIFY_REG(GPIOD->OSPEEDR, (0x3u << 2*13)                   /* PD13 Output High speed                   */
                             , (0x3u << 2*13));
    MODIFY_REG(GPIOD->PUPDR,   (0x3u << 2*13)                   /* PD13 No pull-up, pull-down               */
                           ,   (0x0u << 2*13));

    /*------------------------------------------ Init TIM3_CH1(PB4) ----------------------------------------*/
    MODIFY_REG(RCC->AHB1ENR,   0                                /* Enable GPIOB clock                       */
                           ,   RCC_AHB1ENR_GPIOBEN);
    MODIFY_REG(GPIOB->MODER,   (0x3u << 2*4)                    /* PB4 to AF2(TIM3_CH1)                     */
                           ,   (0x2u << 2*4));
    MODIFY_REG(GPIOB->AFR[0],  (0xFu << 4*4)
                            ,  (0x2u << 4*4));
    MODIFY_REG(GPIOB->PUPDR,   (0x3u << 2*4)                    /* PB4 Pull-Up                              */
                           ,   (0x1u << 2*4));

    /*------------------------------------------ Init Timer 3 ----------------------------------------------*/
    TIME_ARR[0] = ((HAL_RCC_GetPCLK1Freq() * (11 + 1) + (BaudRate / 2)) / BaudRate) - 1;
    TIME_ARR[1] = ((HAL_RCC_GetPCLK1Freq() * (11 + 9) + (BaudRate / 2)) / BaudRate) - 1;
...
{
//  if( (TIM3->SR & TIM_SR_TIF) && (TIM3->DIER & TIM_DIER_TIE) ) {
//      TIM3->SR = ~TIM_SR_TIF;                 /* Timer Trigger interrupt Flag clear       */
//      if( !(GPIOB->IDR & (0x1u << (1 * 4))) ) {
//          PBDP_UART_IDEL_CHK_DS();            /* Timer Counter disable                    */
//      }
//  }
