...
#   define  GOTO_DEL(ret)   { result = ret;  goto RECV_DEL; }
    ...
    if( osSemaphoreWait(PBDP_Info.rx_sem, osWaitForever) <= 0) { GOTO_RET(0); } /* Waiting for Received ED  */
...
//      printf(   "idl_cnt1: %u, idl_cnt2 %u\r\n"
//              , idle[0], idle[1]
//            );
//      printf(   "buff: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X\r\n"
//              , (int)buff[0], (int)buff[1], (int)buff[2], (int)buff[3], (int)buff[4], (int)buff[5], (int)buff[6]
//            );
        return( -5 );
    }
