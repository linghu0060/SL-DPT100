...
#define PBDP_ENTER_RECV_STA()   { if(PBDP_Info.tx_num != 0) PBDP_Info.tx_buf = NULL; }
#define PBDP_STATUS_IS_SEND()   ( (PBDP_Info.tx_num != 0) && (PBDP_Info.tx_buf != NULL) )
...
void PBDP_UART_RecvCB(int ch)
{
    ...
    if( PBDP_STATUS_IS_SEND() ) {   /*------------------ PBDP is Send status ---------------------------*/
    ...
    else {                          /*------------------ PBDP is Recv status ---------------------------*/
        PBDP_RX_QUE_PUSH(ch & 0xFF);                            /* Insert to Receiver queue             */
...
void PBDP_UART_EventCB(int event)
{
    ...
    if( event & PBDP_EVENT_IDLE ) { /*-------------- PBDP Event Recv Idle char ---------------------*/
        if(!PBDP_STATUS_IS_SEND() && (PBDP_Info.idl_cnt == 0) ) {
            PBDP_RX_QUE_PUSH(PBDP_FRAME_ED);                    /* Insert to Receiver queue         */
            osSemaphoreRelease(PBDP_Info.rx_sem);               /* Increase Counter of Received ED  */
        }
        ...
        if( (PBDP_Info.tx_num == 0) && (PBDP_Info.tx_buf != NULL) &&
            (PBDP_Info.tx_chk != 0) && (PBDP_Info.tx_chk <= PBDP_Info.idl_cnt) && (PBDP_Info.tx_cnt != 0) )
        {                                                       /*----- Enter the Send status ------*/
...
#define PBDP_UART_IDEL_CHK_START() // ( TIM3->CR1 |=  TIM_CR1_CEN,      /* Timer Counter enable               */ \
...
#define PBDP_UART_IDEL_CHK_STOP()  // ( TIM3->CR1 &= ~TIM_CR1_CEN,      /* Timer Counter disable              */ \
                                      TIM3->SR   = ~TIM_SR_UIF,       /* Timer Update interrupt flag clear  */ \
                                      NVIC_ClearPendingIRQ(TIM3_IRQn) /* Timer Pending Interrupt clear      */ \
                                    )
...
void PBDP_UART_Init(uint32_t BaudRate)
{
    ...
    MODIFY_REG(TIM3->ARR,      0xFFFF                           /* Auto-reload value                        */
                        ,      (((HAL_RCC_GetPCLK1Freq() * (11 + 0) + (BaudRate / 2)) / BaudRate) - 1));
    ...
    MODIFY_REG(TIM3->CCER,     0xFFFF, 0
                             | TIM_CCER_CC1NP                   /* 00: noninverted/rising edge              */
                             | TIM_CCER_CC1P                    /* 00: noninverted/rising edge              */
                          /* | TIM_CCER_CC1E */ );              /* Capture disabled                         */
    MODIFY_REG(TIM3->DIER,     0xFFFF
